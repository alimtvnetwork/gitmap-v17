...
--- FAIL: TestBuildAuditNeedlesWidthCrossing
    replaceaudit_test.go:63: needle[6] = "gitmap-v17", want "gitmap-v17"
--- FAIL: TestRemoteSlugRegex
    replaceversionparse_test.go:57: regex "gitmap-v17" -> base="gitmap" num="13", want base="gitmap" num="12"
```
...
"gitmap-v17": {true, "gitmap", "12"},   // ← "12" was the captured num for v12
```
...
want := []string{
    ...
    "gitmap-v17", "gitmap/v12",  // ← only "gitmap-v17" got bumped
}
```

When fix-repo rewrites `gitmap-v17` → `gitmap-v17`, the sibling `"12"` / `12` remains, leaving an internally inconsistent test that fails on the next `go test` run.
...
3. The audit gives a precise diagnostic (`paired-literal desync at file:line: 'gitmap-v17' followed by sibling '12'`) instead of a downstream test failure.
